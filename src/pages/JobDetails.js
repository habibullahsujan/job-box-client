...
const JobDetails = () => {
  const { id } = useParams();
  const {
    user: { email, firstName, lastName, _id: applicantId },
  } = useSelector((state) => state.auth);
  const { data } = useGetJobByIdQuery(id, {
    pollingInterval: 5000,
  });
  const totalApplication = data?.data?.totalApplication;

  const { user } = useSelector((state) => state.auth);
  const navigate = useNavigate();
  const [addReply] = useReplyMutation();
  const [applyAJob] = useApplyAJobMutation();
  ...
  const [sendQuestion] = useAddQuestionsMutation();
  const [reply, setReply] = useState("");
  const alreadyAppliedCandidate = data?.data.applicants.find(
    (job) => job.email === email
  );
  ...
  return (
    <div className="pt-14 grid grid-cols-12 gap-5 w-[95%] mx-auto">
        ...
        <div className="space-y-5">
          <div className="flex justify-between items-center mt-5">
            ...
            <button
              ...
              disabled={alreadyAppliedCandidate}
            >
              {alreadyAppliedCandidate ? "Already Applied" : "Apply"}
            </button>
          </div>
          <div>
